...
function CarouselPrevious({ className, ...props }: React.ComponentProps<"button">) {
  ...
  return (
    <button
      className={cn(
        "absolute size-10 rounded-full flex items-center justify-center border transition-all",
        "border-(--gold) text-(--gold) hover:bg-(--gold) hover:text-(--forest)",
        "disabled:opacity-30 disabled:cursor-not-allowed",
        orientation === "horizontal"
          ? "left-0 top-1/2 -translate-y-1/2 -translate-x-12"
  ...
  return (
    <button
      className={cn(
        ...
        "disabled:opacity-30 disabled:cursor-not-allowed",
        orientation === "horizontal"
          ? "right-0 top-1/2 -translate-y-1/2 translate-x-12"
